...
function MetadataStrip({ metadata }: { metadata: RunMetadata }) {
  return (
    <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
      ...
      <MetadataPill>
        <span className="uppercase tracking-[0.18em]">Run</span>
...
export function AnswerCard({ answer, index }: { answer: Answer; index: number }) {
  return (
    <article className="border-t border-border pt-6">
      <div className="grid gap-4 grid-cols-[72px_minmax(0,1fr)] md:grid-cols-[84px_minmax(0,1fr)]">
        <div className="text-center">
          <div className="border border-border bg-muted/30 px-2 py-3 md:px-3">
            <p className="text-3xl font-semibold leading-none">{answer.score}</p>
